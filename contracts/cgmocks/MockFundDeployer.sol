...
import "../release/core/fund/comptroller/IComptroller.sol";
import "../release/core/fund/comptroller/ComptrollerProxy.sol";
import "../release/core/fund/vault/IVault.sol";
import "../release/core/fund-deployer/IFundDeployer.sol";

/// @title MockFundDeployer Contract
/// @notice This contract is Mocked contract of FundDeployer.sol
/// to assume this contract has some defects on invokeMigration hooks
contract MockFundDeployer is IFundDeployer, IMigrationHookHandler {
    ...
    modifier onlyFndzController() {
        require(msg.sender == FNDZ_CONTROLLER, "Only the FNDZ Controller can call this function");
        _;
    }
    ...
    /// @notice Set the vault lib address only once after contract deployment
    /// to break the contract deployment dependancy cycle caused by adding
    /// AssetFinalityResolver to the VaultLib
    /// @param _vaultLib The VaultLib contract address
    /// @dev Can only be set once
    function setVaultLib(address _vaultLib) external onlyOwner {
        require(vaultLib == address(0), "setVaultLib: This value can only be set once");
    ...
    /// Ownership is claimed when the owner of the Dispatcher contract (the FNDZ Council)
    ...
    function invokeMigrationInCancelHook(
        ...
        require(false, "Reverted for testing");
        return;
    }
