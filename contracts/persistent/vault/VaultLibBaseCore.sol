...
import "../../fndz/utils/InlineSwapMixin.sol";
...
abstract contract VaultLibBaseCore is
    IMigratableVault,
    ProxiableVaultLib,
    SharesTokenBase,
    InlineSwapMixin
{
    ...
    event FNDZControllerSet(address prevFNDZController, address nextFNDZController);
    ...
    address internal fndzController;
    ...
    function init(
        ...
        __setOwner(_owner);
        __setFNDZController(_fndzController);
    ...
    /// @dev Helper to set the FNDZController of the VaultProxy
    function __setFNDZController(address _nextFNDZController) internal {
        require(
            _nextFNDZController != address(0),
            "__setFNDZController: _nextFNDZController cannot be empty"
        );
        address prevFNDZController = fndzController;
        require(
            _nextFNDZController != prevFNDZController,
            "__setFNDZController: _nextFNDZController is the current fndzController"
        );

        fndzController = _nextFNDZController;

        emit FNDZControllerSet(prevFNDZController, _nextFNDZController);
    }
}
