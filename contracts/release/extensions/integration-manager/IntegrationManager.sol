    ...
    PermissionedVaultActionMixin
{
    ...
    address private immutable FNDZ_INVESTMENT_REGISTRY;
    ...
    {   
        return
            ...
            _who == IFNDZController(FNDZ_CONTROLLER).getFNDZShortingBotAddress();
    }
    ...
    function __addTrackedAssets(bytes memory _callArgs) private {
        address[] memory assets = abi.decode(_callArgs, (address[]));
        for (uint256 i; i < assets.length; i++) {
            require(__isSupportedAsset(assets[i]), "__addTrackedAssets: Unsupported asset");
        ...
        )
    {
        ...
        address fndzToken = IFNDZInvestmentRegistry(FNDZ_INVESTMENT_REGISTRY).fndzToken();
        ...
        for (uint256 i = 0; i < spendAssets_.length; i++) {
            ...
            if (!expectedIncomingAssets_.contains(spendAssets_[i])) {
                preCallSpendAssetBalances_[i] = ERC20(spendAssets_[i]).balanceOf(
                    address(vaultProxyContract)
                );
            }

            // Check to ensure that locked FNDZ are not being spent
            if (spendAssets_[i] == fndzToken) {
                uint256 fndzBalance = ERC20(fndzToken).balanceOf(address(vaultProxyContract));
                require(
                    maxSpendAssetAmounts_[i] <=
                        fndzBalance.sub(
                            IFNDZInvestmentRegistry(FNDZ_INVESTMENT_REGISTRY).getLockedAmount(
                                address(vaultProxyContract)
                            )
                        ),
