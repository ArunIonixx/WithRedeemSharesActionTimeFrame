...
/// @title MockFixedRateManagementFee Contract
/// @notice A management fee with a fixed annual rate, used for testing
/// the FNDZController's support for zero-parameter fees
contract MockFixedRateManagementFee is FeeBase, MakerDaoMath {
    ...
    function addFundSettings(address _comptrollerProxy, bytes calldata)
        ...
        onlyFeeManager
    {
        uint256 scaledPerSecondRate = 1000000000318694059332284760;
        ...
        )
    {
        ...
        implementedHooksForSettle_[1] = IFeeManager.FeeHook.BuySharesCompleted;
        ...
        )
    {
        ...
        if (feeInfo.lastSettled == 0) {
            // Management fee should not be paid out on first action on vault since
            // lastSettled will be zero, thus secondsSinceSettlement will be too high
            // and cause a math overflow. When the fee is settled for the first time,
            // just save the current timestamp to serve as a point of reference for
            // future payouts.
            comptrollerProxyToFeeInfo[_comptrollerProxy].lastSettled = block.timestamp;
        ...
        return (IFeeManager.SettlementType.MintAndSplit, address(0), sharesDue_);
    }
