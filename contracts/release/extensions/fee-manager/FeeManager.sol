    ...
    PermissionedVaultActionMixin
{
    ...
    // FNDZ Contract Addresses
    address private immutable FNDZ_CONTROLLER;
    ...
    function receiveCallFromComptroller(
        ...
        if (_actionId == 0) {
            ...
            // It allows to execute logic updates after any processes
            // (i.e, FNDZInvestmentFee locked amount update after performance fee payout)
            __invokeHook(msg.sender, IFeeManager.FeeHook.PostContinuous, "", 0, true);
        } else {
    ...
    function __forcePayoutAllSharesOutstanding(address _comptrollerProxy) private {
        ...
        __splitAndTransferSharesOutstanding(
            _comptrollerProxy,
            vaultProxy,
            payee,
    ...
    function __getGavAsNecessary(
        ...
        if (_gavOrZero == 0 && feeUsesGavOnUpdate(_fee)) {
            ...
            bool gavIsValid;
            (gav_, gavIsValid) = IComptroller(_comptrollerProxy).calcGav();
    ...
    /// @notice Helper method to get the performance fee splits from FNDZController and
    /// and transfers the shares and assets accordingly
    function __splitAndTransferSharesOutstanding(
        address _comptrollerProxy,
        address _vaultProxy,
        address _vaultOwner,
        uint256 _totalSharesDue
    ) private {
        (
            address stakingPool,
            address fndzDao,
            uint256 vaultOwnerSplit,
            uint256 stakingAndDaoSplit,
            uint256 rateDivisor
        ) = IFNDZController(FNDZ_CONTROLLER).getPerformanceFeeData(_vaultOwner);
        uint256 vaultOwnerSharesDue = _totalSharesDue.mul(vaultOwnerSplit).div(rateDivisor);
        uint256 stakingPoolSharesDue = _totalSharesDue.mul(stakingAndDaoSplit).div(rateDivisor);
        uint256 fndzDaoSharesDue = _totalSharesDue - vaultOwnerSharesDue - stakingPoolSharesDue;
        // Since this function is called, we are assured _totalSharesDue > 0
        // But, after performing the above calculations, it's possible that
        // a beneficiary's share amount is rounded down to zero
        if (vaultOwnerSharesDue > 0) {
            __transferShares(_comptrollerProxy, _vaultProxy, _vaultOwner, vaultOwnerSharesDue);
        }
        if (stakingPoolSharesDue > 0) {
            __burnShares(_comptrollerProxy, _vaultProxy, stakingPoolSharesDue);
            __redeemVirtualShares(_comptrollerProxy, stakingPool, stakingPoolSharesDue);
        }
        if (fndzDaoSharesDue > 0) {
            __burnShares(_comptrollerProxy, _vaultProxy, fndzDaoSharesDue);
            __redeemAndSwapVirtualShares(_comptrollerProxy, fndzDao, fndzDaoSharesDue);
        }
    }
    ...
    function __settleFee(
        ...
        if (settlementType == SettlementType.Direct) {
            ...
            payer = _vaultProxy;
            __burnShares(_comptrollerProxy, payer, sharesDue);
        } else if (settlementType == SettlementType.MintAndSplit) {
            payee = IVault(_vaultProxy).getOwner();
            __mintAndSplitHelper(_comptrollerProxy, payee, sharesDue);
        } else if (settlementType == SettlementType.DirectToReferrer) {
            payee = IReferralRegistry(REFERRAL_REGISTRY).getReferrer(_vaultProxy, payer);
            __transferShares(_comptrollerProxy, payer, payee, sharesDue);
        } else if (settlementType == SettlementType.BuyToLockFNDZ) {
            address fndzToken = IFNDZController(FNDZ_CONTROLLER).fndzToken();
            address denominationAsset = IComptroller(_comptrollerProxy).getDenominationAsset();
            __swapAsset(_comptrollerProxy, denominationAsset, fndzToken, sharesDue);
        } else {
    ...
    /// @dev Helper to handle split management fee
    function __mintAndSplitHelper(
        address _comptrollerProxy,
        address _vaultOwner,
        uint256 _totalSharesDue
    ) private {
        (
            ...
            address fndzDao,
            uint256 vaultOwnerSplit,
            uint256 stakingPoolSplit,
            uint256 rateDivisor
        ) = IFNDZController(FNDZ_CONTROLLER).getManagementFeeData();
        uint256 vaultOwnerSharesDue = _totalSharesDue.mul(vaultOwnerSplit).div(rateDivisor);
        uint256 stakingPoolSharesDue = _totalSharesDue.mul(stakingPoolSplit).div(rateDivisor);
        uint256 fndzDaoSharesDue = _totalSharesDue - vaultOwnerSharesDue - stakingPoolSharesDue;
        // Since this function is called, we are assured _totalSharesDue > 0
        // But, after performing the above calculations, it's possible that
        // a beneficiary's share amount is rounded down to zero
        if (vaultOwnerSharesDue > 0) {
            __mintShares(_comptrollerProxy, _vaultOwner, vaultOwnerSharesDue);
        }
        if (stakingPoolSharesDue > 0) {
            __redeemVirtualShares(_comptrollerProxy, stakingPool, stakingPoolSharesDue);
        }
