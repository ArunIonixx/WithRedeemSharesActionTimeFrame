...
import "../fndz/interfaces/IFNDZStaking.sol";
import "../release/core/fund-deployer/IFundDeployer.sol";

contract MockFNDZControllerV2 is IFNDZController, OwnableUpgradeable {
    ...
    event FndzStakingPoolUpdated(address _oldStaking, address _newStaking);
    ...
    event NewStateVariableUpdated(uint256 _value);
    ...
    /// @param _fndzStakingPool The address of the FNDZ Staking Pool contract
    ...
    function initialize(
        ...
        emit FndzStakingPoolUpdated(address(0), _fndzStakingPool);
    ...
    /// swapped to during inline fee share redemption
    /// @param _fndzDaoDesiredToken The address of the token in which the FNDZ DAO wishes to collect fee payments
